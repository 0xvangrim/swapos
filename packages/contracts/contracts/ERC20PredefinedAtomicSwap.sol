 ...
 */
contract ERC20PredefinedAtomicSwap {
    ...
    returns (bytes32 contractId)
  {
    ...
    contractId = sha256(
      abi.encodePacked(msg.sender, _timelock, _senderToken, _senderAmount, _receiverToken, _receiverAmount)
    );
  ...
  function withdraw(
    ...
    returns (bytes32 counterContractId)
  {
    LockContract storage c = contracts[_contractId];


    require(
      ERC20(c.receiverToken).allowance(msg.sender, address(this)) >= c.receiverAmount,
      'tokensTransferable: token allowance must be >= amount'
    );

    
    if (!ERC20(c.receiverToken).transferFrom(msg.sender, address(this), c.receiverAmount)) {
      revert('withdraw: transferFrom sender to this failed');
    }

    counterContractId = sha256(
      abi.encodePacked(msg.sender, c.timelock, c.receiverToken, c.receiverAmount, c.senderToken, c.senderAmount)
    );

    contracts[counterContractId] = LockContract(
      msg.sender,
      c.receiverToken,
      c.receiverAmount,
      c.senderToken,
    ...
    c.counterContractId = counterContractId;
    c.withdrawn = true;
    ERC20(c.senderToken).transfer(c.receiver, c.senderAmount);
    ERC20(c.receiverToken).transfer(c.sender, c.receiverAmount);
    emit HTLCERC20Withdrawn(counterContractId);
  }
    ...
    )
  {
    if (haveContract(_contractId) == false)
      return (address(0), address(0), 0, address(0), 0, 0, false, false, address(0), 0x0);
    LockContract storage c = contracts[_contractId];
    return (
      c.sender,
      c.senderToken,
